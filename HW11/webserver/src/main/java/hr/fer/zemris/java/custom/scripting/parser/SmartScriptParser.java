...
public class SmartScriptParser {
	/**
	 * Document body created after reading the document.
	 */
	...
	/**
	 * Integer representing DFA states. 0 - starting state, 1 - text state, 2 -
	 * tag state, 3 - escape state
	 */
	private int state = 0;
	 ...
	 *
	 * @param documentBody
	 *            String that contains the document body.
	 */
	...
	private void parseTheDocument() throws SmartScriptParserException {
		...
		while (documentBody.length() != 0) {
			...
			case 1:
				if (c == '{') {
					...
					final String temp = documentBody.substring(1);
					final char tempchar = temp.charAt(0);
			...
			case 2:
				if (c == '$') {
					...
					if (builder.length() != 0) {
						final TextNode text = new TextNode(builder.toString());
						final Node temp = (Node) stack.peek();
					...
					throw new SmartScriptParserException(
							"Document contains an invalid expression!");
				}
				break;
		...
		// if there is something else on the stack, the document structure is
		// invalid
		if (stack.size() > 1) {
			throw new SmartScriptParserException(
					"Error in the document, there weren't enough end tags!");
		} else if (stack.size() < 1) {
			throw new SmartScriptParserException(
					"Error in the document, there were too many end tags.");
		}
	}
	 ...
	 *
	 * @throws SmartScriptParserException
	 *             if an error occurs while parsing document.
	 */
	private void createForNode() throws SmartScriptParserException {
		...
		final TokenVariable variable = new TokenVariable(
				documentBody.substring(0, documentBody.indexOf(" ")));
		...
		final String temp = documentBody
				.substring(0, documentBody.indexOf("$"));
		...
		if (str.length == 1) {
			...
			if (str.length < 2 || str.length > 3) {
				throw new SmartScriptParserException(
						"For loop must have three or four parameters.");
			}
		}

		final Token[] tokens = new Token[str.length];

		// create tokens from the loop body
		int i = 0;
		for (final String str2 : str) {
			if (str2.contains("\"")) {
				tokens[i] = new TokenString(str2);
			} else if ((str2.charAt(0) == '+' || str2.charAt(0) == '-'
					|| str2.charAt(0) == '*' || str2.charAt(0) == '/')
					&& str2.length() == 1) {
				throw new SmartScriptParserException(
						"An operator can't be a for loop parameter.");
			} else if (str2.charAt(0) == '@') {
				throw new SmartScriptParserException(
						"A function cannot be a for loop parameter!");
		...
		final ForLoopNode flop = new ForLoopNode(variable, tokens);

		final Node temp2 = (Node) stack.peek();
	 ...
	 *
	 * @param builder
	 *            StringBuilder with data needed to create a TextNode.
	 */
	private void createTextNode(final StringBuilder builder) {
		final TextNode text = new TextNode(builder.toString());
		builder.setLength(0);
		final Node temp = (Node) stack.peek();
		temp.addChildNode(text);
	...
	private void createEchoNode() {
		...
		final String[] temp2 = temp.split("\\s+");
		...
		final Token[] tokens = new Token[temp2.length];

		// create tokens from loop body
		int i = 0;
		for (final String str : temp2) {
			if (str.contains("\"")) {
				...
				final double temp3 = Double.parseDouble(str);
				tokens[i] = new TokenConstantDouble(temp3);
			} else {
		...
		final Node temp3 = (Node) stack.peek();
	...
	public DocumentNode getDocumentNode() throws SmartScriptParserException {
		final DocumentNode temp = (DocumentNode) stack.pop();
